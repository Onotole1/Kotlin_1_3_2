...
fun calcVisaPeace(transferSize: Int): Int {
    return if (transferSize * val0075 < val35) transferSize - val35 else (transferSize * (1 - val0075)).toInt();
}

fun calcMastercardMaestro(thisMonthTransfers: Int, transferSize: Int): Int {
    return if (thisMonthTransfers <= val75000) transferSize else (transferSize * (1 - val006) - val20).toInt();
}
